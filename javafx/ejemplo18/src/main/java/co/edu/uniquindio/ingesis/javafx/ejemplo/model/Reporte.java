...
public class Reporte {
    ...
    public Reporte(String nombre, List<EstudianteNotaDTO> estudiantes) {
        ...
        DoubleSummaryStatistics statistics = estudiantes.stream().mapToDouble(EstudianteNotaDTO::getNota).summaryStatistics();
        notaMaxima = (float) statistics.getMax();
        notaMinima = (float) statistics.getMin();
        notaPromedio = (float) statistics.getAverage();
        numeroPerdieron = (int) estudiantes.stream().mapToDouble(EstudianteNotaDTO::getNota).filter(nota->nota<3).count();
